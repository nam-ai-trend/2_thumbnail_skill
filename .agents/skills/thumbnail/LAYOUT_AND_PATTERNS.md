# LAYOUT_AND_PATTERNS.md

## 📐 레이아웃 렌더링 절대 규칙
- 썸네일 해상도는 무조건 `1280x720`으로 고정한다 (`w-[1280px] h-[720px]`).
- 컴포넌트 간의 여백(Gap)과 패딩(Padding)은 임의로 수정할 수 없으며, 명세된 수치만 사용한다.
- **[NEW]** 배경은 단색 대신 풍부한 그라데이션이나 AI로 생성한 고품질 배경 이미지를 적극 권장한다.
